...
Public Sub TraceListPop()
    ...
    If stackList.Count > 0 Then
        stackList.RemoveAt stackList.Count - 1 ' 最後の要素を削除
    End If
If stackList.Count = 0 Then
        Set stackList = Nothing
        stackTrace_ = ""
...
'----------------------------------------------------------------------------------------------------+
'----------Title：UpdateStackTraceText
'----------概要：トレースリストを文字列に書き起こすサブルーチン
'-----Arg：なし
'----------------------------------------------------------------------------------------------------+
Private Sub UpdateStackTraceText()
    If stackList Is Nothing Or stackList.Count = 0 Then
        stackTrace_ = ""
    Else
        stackTrace_ = Join(stackList.ToArray(), " -> ")
    End If
End Sub